...
// fails its engine check without a word, so a floor raised past the pin breaks the deploy and nothing
// local. The ranges these packages publish are ^x.y.z and >=x.y.z joined by ||; anything else throws
